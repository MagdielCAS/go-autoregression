...
func TestNewLSARXPredictor(t *testing.T) {
	testCases := []struct {
		name        string
		data        [][]float64
		params      LSARXModelParameters
		expectedErr bool
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := NewLSARXPredictor(tc.data, tc.params)
			if (err != nil) != tc.expectedErr {
				t.Errorf("NewLSARXPredictor() error = %v, expectedErr %v", err, tc.expectedErr)
			}
		})
	}
}
...
func TestPredict(t *testing.T) {
	...
	numToPredict := 3
	predictor, err := NewLSARXPredictor(data, params)
	if err != nil {
